Test.run 'Method (class)',
  ...
  tests:
    'has default values upon construction': (test) ->
      fn = ->
      method = new BDD.Method('does something', fn)
    ...
    'is async': (test) ->
      method = new BDD.Method('does something', (done) ->)
...
Test.run 'Method (run asynchronous)',
  ...
  tests:
    ...
    'times out (error)': (test, done) ->
      self = undefined
      method = new BDD.Method('foo', (done) ->) # Callback not invoked.
      method.timeout = 200
      err = null
      runAsync = (callback) ->
          method.run null, (e) ->
